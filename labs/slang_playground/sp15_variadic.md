# sp15_variadic — Variadic Generics

Interface-constrained tintable args as color bands.
...
```powershell
python -m slang_falcon.live --lesson slang_playground/sp15_variadic
python -m slang_falcon.live --lesson slang_playground/sp15_variadic --once
```

## Try

1. Open the matching demo in the [playground](https://shader-slang.org/slang-playground/) and compare.
2. Edit `shaders/sp15_variadic.slang` — save to hot-reload.
3. Note API differences: playground uses `import rendering` / `drawPixel`; we use `hello_pixel(pixel, resolution, time)`.

Shader: `shaders/sp15_variadic.slang`
