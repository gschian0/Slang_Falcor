# sp10_generics — Generics & Extensions

IMyHashable Circle + float3 via extensions.
...
```powershell
python -m slang_falcon.live --lesson slang_playground/sp10_generics
python -m slang_falcon.live --lesson slang_playground/sp10_generics --once
```

## Try

1. Open the matching demo in the [playground](https://shader-slang.org/slang-playground/) and compare.
2. Edit `shaders/sp10_generics.slang` — save to hot-reload.
3. Note API differences: playground uses `import rendering` / `drawPixel`; we use `hello_pixel(pixel, resolution, time)`.

Shader: `shaders/sp10_generics.slang`
