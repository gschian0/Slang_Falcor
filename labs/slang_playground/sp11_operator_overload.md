# sp11_operator_overload — Operator Overload

MyPoint + and [] visualized.
...
```powershell
python -m slang_falcon.live --lesson slang_playground/sp11_operator_overload
python -m slang_falcon.live --lesson slang_playground/sp11_operator_overload --once
```

## Try

1. Open the matching demo in the [playground](https://shader-slang.org/slang-playground/) and compare.
2. Edit `shaders/sp11_operator_overload.slang` — save to hot-reload.
3. Note API differences: playground uses `import rendering` / `drawPixel`; we use `hello_pixel(pixel, resolution, time)`.

Shader: `shaders/sp11_operator_overload.slang`
