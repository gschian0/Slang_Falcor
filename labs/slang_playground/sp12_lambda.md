# sp12_lambda — Lambda Expressions

MyData.map with a capturing lambda as bars.
...
```powershell
python -m slang_falcon.live --lesson slang_playground/sp12_lambda
python -m slang_falcon.live --lesson slang_playground/sp12_lambda --once
```

## Try

1. Open the matching demo in the [playground](https://shader-slang.org/slang-playground/) and compare.
2. Edit `shaders/sp12_lambda.slang` — save to hot-reload.
3. Note API differences: playground uses `import rendering` / `drawPixel`; we use `hello_pixel(pixel, resolution, time)`.

Shader: `shaders/sp12_lambda.slang`
