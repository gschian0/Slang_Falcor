# sp09_properties — Properties

Packed MyPoint x/y properties as moving markers.
...
```powershell
python -m slang_falcon.live --lesson slang_playground/sp09_properties
python -m slang_falcon.live --lesson slang_playground/sp09_properties --once
```

## Try

1. Open the matching demo in the [playground](https://shader-slang.org/slang-playground/) and compare.
2. Edit `shaders/sp09_properties.slang` — save to hot-reload.
3. Note API differences: playground uses `import rendering` / `drawPixel`; we use `hello_pixel(pixel, resolution, time)`.

Shader: `shaders/sp09_properties.slang`
